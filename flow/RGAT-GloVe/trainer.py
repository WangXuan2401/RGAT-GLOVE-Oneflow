...
class ABSATrainer(object):
    def __init__(self, args, emb_matrix=None):
        ...
        self.model.cuda()
        self.optimizer = flow_utils.get_optimizer(args.optim, self.parameters, args.lr)

    # load model_state and args
    def load(self, filename):
        try:
            checkpoint = flow.load(filename)
    ...
    def update(self, batch):
        ...
        corrects = (flow.max(logits, 1)[1].view(label.size()).data == label.data).sum()
