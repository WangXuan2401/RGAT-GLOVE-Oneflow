...
class ABSADataLoader(object):
    ...
    def __getitem__(self, key):
        ...
        # length
        length = flow.LongTensor(batch[7])
        # label
        label = flow.LongTensor(batch[8])
...
def get_long_tensor(tokens_list, batch_size):
    ...
    tokens = flow.LongTensor(batch_size, token_len).fill_(0)
    for i, s in enumerate(tokens_list):
        tokens[i, : len(s)] = flow.LongTensor(s)
    return tokens
...
def get_float_tensor(tokens_list, batch_size):
    ...
    token_len = max(len(x) for x in tokens_list)
    tokens = flow.FloatTensor(batch_size, token_len).fill_(0)
    for i, s in enumerate(tokens_list):
        tokens[i, : len(s)] = flow.FloatTensor(s)
    return tokens
