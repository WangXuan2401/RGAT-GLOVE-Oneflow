...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(
            ...
            if type == "self":
                ...
                if layer_cache is not None:
                    ...
                    if layer_cache["self_keys"] is not None:
                        key = flow.cat((layer_cache["self_keys"].to(device), key), dim=2)
                    if layer_cache["self_values"] is not None:
                        value = flow.cat((layer_cache["self_values"].to(device), value), dim=2)
        ...
        scores = flow.matmul(query, key.transpose(2, 3))  # [batch_size, nhead, seq_len, seq_len]
        ...
        context = flow.matmul(drop_attn, value)

        if structure_v is not None:
            drop_attn_v = drop_attn.transpose(1, 2)
            context_v = flow.matmul(drop_attn_v, structure_v)
