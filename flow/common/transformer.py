...
from flow.nn.init import xavier_uniform_
...
def build_encoder(opt, embeddings):
  ...
  """
    return TransformerEncoder(
        opt.enc_layers, opt.enc_rnn_size, opt.heads, opt.transformer_ff, opt.dropout, embeddings
    )
...
def build_decoder(opt, embeddings):
  ...
  """
    return TransformerDecoder(
        opt.dec_layers, opt.dec_rnn_size, opt.heads, opt.transformer_ff, opt.dropout, embeddings
    )
...
def load_test_model(opt, dummy_opt, model_path=None):
    ...
    checkpoint = flow.load(model_path, map_location=lambda storage, loc: storage)
...
def build_base_model(model_opt, fields, gpu, checkpoint=None):
  ...
  """
    ...
    device = flow.device("cuda:0" if gpu else "cpu")
